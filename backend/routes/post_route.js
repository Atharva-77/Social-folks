...
router.get("/allpost", async (req, res) => {
  try {
    ...
    const data = await PostDb.find({})
      .populate("postedBy", "Name username email profilePicUrl")
      .populate("originalPostedBy", "Name username email profilePicUrl")
      .populate("retweetDataId")
      .populate("replyDataId")
      .sort({ createdAt: -1 });
    // console.log("DATA",data);
    // const data
    const data1 = await RegisterDb.populate(data, {
      path: "retweetDataId.postedBy",
    });
    // console.log("\n\nALLPOST\n\n",(data1),typeof(data1));
    const data2 = await RegisterDb.populate(data1, {
      path: "replyDataId.postedBy",
    });
    try {
...
router.get("/allpost/following/:id", protect, async (req, res) => {
  try {
    ...
    const data = await PostDb.find({ postedBy: getUser.following })
      .populate("postedBy", "Name username email profilePicUrl")
      .populate("originalPostedBy", "Name username email profilePicUrl")
      .populate("retweetDataId")
      .populate("replyDataId")
...
router.get("/specificPost", async (req, res) => {
  try {
    ...
    console.log(
      "ENTERED Specific POST",
      keyword,
      keyword.length,
      req.query.keyword.trim().length
    );
    ...
    const data_Content = await PostDb.find(searchWord_Content)
      .populate("postedBy", "Name username email profilePicUrl")
      .populate("originalPostedBy", "Name username email profilePicUrl")
      .populate("retweetDataId")
      .populate("replyDataId")
    ...
    const dataRetweet_Content = await PostDb.find({
      ...searchWord_retweetContent,
    })
      .populate("postedBy", "Name username email profilePicUrl")
      .populate("originalPostedBy", "Name username email profilePicUrl")
      .populate("retweetDataId")
    ...
    // const data
    const data_Content1 = await RegisterDb.populate(data_Content, {
      path: "retweetDataId.postedBy",
    });
    const data_Content2 = await RegisterDb.populate(data_Content1, {
      path: "replyDataId.postedBy",
    });

    const dataRetweet_Content1 = await RegisterDb.populate(
      dataRetweet_Content,
      { path: "retweetDataId.postedBy" }
    );
    const dataRetweet_Content2 = await RegisterDb.populate(
      dataRetweet_Content1,
      { path: "replyDataId.postedBy" }
    );
    ...
    if (searchQuery_result.length == 0) searchQuery_result = "No Such Posts";
...
router.get("/specificUsers", async (req, res) => {
  try {
    ...
    const data_searchName = await RegisterDb.find(searchWord_Name).select(
      "-password -followers -following"
    );
    const data_searchUsername = await RegisterDb.find(
      searchWord_Username
    ...
    if (searchQuery_result.length == 0) searchQuery_result = "Invalid Details";
    else {
...
router.get("/postedBy/:id", async (req, res) => {
  try {
    ...
    const postDetail = await PostDb.find({ postedBy: getUser._id })
      .populate("postedBy", "Name username email profilePicUrl")
      .populate("originalPostedBy", "Name username email profilePicUrl")
      .populate("retweetDataId")
      .populate("replyDataId")
    ...
    const data1 = await RegisterDb.populate(postDetail, {
      path: "retweetDataId.postedBy",
    });
    const data2 = await RegisterDb.populate(data1, {
      path: "replyDataId.postedBy",
...
router.get("/postedBy/likes/:id", async (req, res) => {
  try {
    ...
    const getUser = await RegisterDb.findOne({ username: username }).populate(
      "likes"
    ); //.sort({"createdAt":-1})

    // console.log("GETUSER",getUser);
    const data0 = await RegisterDb.populate(getUser, {
      path: "likes.postedBy",
    });
    ...
    const data1 = await RegisterDb.populate(data0.likes, {
      path: "retweetDataId",
    });
    // console.log("\n\n1.Likes GETUser",data1);

    const data2 = await RegisterDb.populate(data1, { path: "replyDataId" });
    const data3 = await RegisterDb.populate(data1, {
      path: "replyDataId.postedBy",
    });
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const postDetail = await PostDb.findById(postid)
      .populate("postedBy", "Name username email profilePicUrl")
      .populate("originalPostedBy", "Name username email profilePicUrl")
      .populate("retweetDataId")
      .sort({ createdAt: -1 });
    ...
    const full_postDetail1 = await RegisterDb.populate(postDetail, {
      path: "retweetDataId",
    });
    // console.log("FD2",full_postDetail,"\nPD2",postDetail);
    const full_postDetail2 = await RegisterDb.populate(full_postDetail1, {
      path: "replyDataId",
    });
    const full_postDetail3 = await RegisterDb.populate(full_postDetail2, {
      path: "replyDataId.postedBy",
    });
    try {
...
router.put("/:id/like", protect, async (req, res) => {
  try {
    ...
    const updatedUser = await RegisterDb.findByIdAndUpdate(
      userid,
      { [option]: { likes: postid } },
      { new: true }
    )
    ...
    console.log(
      ...
      isLiked,
      req.userAuth.likes,
      req.userAuth.likes.includes(postid),
      option
    );
...
router.post("/:id/retweet", protect, async (req, res) => {
  ...
  try {
    ...
    const deletedPost = await PostDb.findOneAndDelete({
      postedBy: userid,
      retweetDataId: postid,
    }).catch((err) =>
      res.status(401).json("Delete RETWEET ka Error is " + err)
    );
    ...
    if (deletedPost == null) {
      var data = await PostDb.findById(postid).populate(
        "postedBy",
        "Name username email"
      );
      ...
      var finalPostedBy =
        data.originalPostedBy != undefined
          ? data.originalPostedBy._id
          : data.postedBy._id;

      // console.log("\n\nFINAL-POSTED",finalPostedBy);

      repost = await PostDb.create({
        postedBy: userid,
        retweetDataId: postid,
        retweetContent: data.content,
        originalPostedBy: finalPostedBy,
      })
    ...
    const updatedUser = await RegisterDb.findByIdAndUpdate(
      ...
      // .then(res.status(201).json("Success put "+ isLiked))
      .catch((err) =>
        res.status(401).json("Retweet UserUpdate Error is " + err)
      );
    ...
    const updatedPost = await PostDb.findByIdAndUpdate(
      postid,
      { [option]: { retweetUserList: userid } },
      { new: true }
    )
      // .then(res.status(201).json("Success put "+ isLiked))
      .catch((err) =>
        res.status(401).json("Retweet Post Update Error is " + err)
      );
